...
static inline RBinPEObj* PE_(get)(RBinFile *bf) {
...
R_IPI PE_DWord PE_(va2pa)(RBinPEObj* pe, PE_DWord rva) {
...
int PE_(pe_pe_get_claimed_checksum)(RBinPEObj* pe) {
...
int PE_(pe_pe_get_actual_checksum)(RBinPEObj* pe) {
...
static const char* PE_(pe_pe_get_claimed_authentihash)(RBinPEObj* pe) {
	if (!pe->spcinfo) {
		return NULL;
	}
...
const char* PE_(pe_pe_compute_authentihash)(RBinPEObj* pe) {
...
const char* PE_(pe_pe_get_authentihash)(RBinPEObj* pe) {
	return pe->authentihash;
}

int PE_(pe_pe_is_authhash_valid)(RBinPEObj* pe) {
...
int PE_(pe_pe_get_overlay)(RBinPEObj* pe, ut64* size) {
...
static int bin_pe_init_overlay(RBinPEObj* pe) {
	...
	ut64 pe_overlay_offset = PE_(pe_pe_get_overlay) (pe, &pe_overlay_size);
...
R_API void PE_(pe_pe_parse_resource)(RBinPEObj *pe) {
	int index = 0;
...
static int bin_pe_init_security(RBinPEObj *pe) {
	...
	if (pe->cms && pe->spcinfo) {
		const char *actual_authentihash = PE_(pe_pe_compute_authentihash) (pe);
		const char *claimed_authentihash = PE_(pe_pe_get_claimed_authentihash) (pe);
